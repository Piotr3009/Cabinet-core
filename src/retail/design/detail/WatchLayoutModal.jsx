...
function Schematic({ layout, clear, profile }) {
  ...
  return (
    <svg viewBox={`-2 -2 ${W + 4} ${H + 4}`} className="pbi-re-wfull" role="img" aria-label={layout.label}>
      <rect x="-2" y="-2" width={W + 4} height={H + 4} rx="2" className="pbi-re-svg-fill-ground pbi-re-svg-stroke-hair" strokeWidth="1" />
      {cells.map((c) => (
        <rect
          key={c.key}
          x={c.x}
          ...
          rx="1"
          className={c.kind === 'pocket' ? 'pbi-re-svg-fill-gold-wash pbi-re-svg-stroke-gold' : 'pbi-re-svg-fill-panel pbi-re-svg-stroke-quiet'}
          strokeWidth="0.8"
        />
      ))}
    </svg>
...
export default function WatchLayoutModal() {
  ...
  return (
    <Modal
      ...
      width="pbi-re-w520"
    >
      <div className="pbi-re-stack-3" data-watch-layout-modal={item.id}>
        ...
        <div className="pbi-re-grid pbi-re-grid-2 pbi-re-gap-2">
          {WATCH_LAYOUTS.map((l) => (
            <button
              ...
              className={`pbi-re-round pbi-re-line pbi-re-p2 pbi-re-left ${chosen.id === l.id
                ? 'pbi-re-hair-gold pbi-re-fill-panel'
                : 'pbi-re-hair-soft pbi-re-hair-hover'}`}
              onClick={() => {
            ...
            >
              ...
              <div className="pbi-re-mt1 pbi-re-tsm pbi-re-ink-1">{l.label}</div>
              <div className="pbi-re-t10 pbi-re-lead-tight pbi-re-quiet">{l.hint}</div>
            </button>
          ))}
        </div>
        ...
        {/* ─── THE FINISH (F8f) ────────────────────────────────────────── */}
        <div className="pbi-re-fieldrow pbi-re-tsm">
          <span className="pbi-re-grow pbi-re-ink-1">Finish</span>
          <div className="pbi-re-row pbi-re-gap-1">
            {[{ id: null, label: 'Project' }, ...WATCH_FINISHES].map((f) => (
              <button
                ...
                className={`pbi-re-btn pbi-re-px2 pbi-re-py05 pbi-re-t11 ${finish === f.id ? 'pbi-re-btn-gold' : ''}`}
        ...
        {built ? (
          <div className="pbi-re-t11 pbi-re-quiet" data-watch-built="1">
          ...
          <div className="pbi-re-t11 pbi-re-warn" data-watch-built="0">
