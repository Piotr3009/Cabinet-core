...
export default function UnitFinishModal() {
  ...
  return (
    <Modal
      ...
      footer={(
        <>
          <button
            type="button"
            className="cc-btn flex-1 text-left"
    ...
    >
      <div className="space-y-3" data-unit-palette="1">
        <Row
          label="Carcass"
          entries={carcass}
  ...
  return (
    <div className="space-y-1">
      <div className="cc-row">
        <span className="cc-label mb-0 flex-1">{label}</span>
        {chosen.mixed && <span className="cc-tag" data-mixed="1">mixed</span>}
        {!chosen.mixed && !chosen.value && <span className="text-[11px] text-ink-400">project</span>}
      </div>
        ...
        {entries.map((p) => {
          ...
          return (
            <button
              ...
              aria-pressed={on}
              className={`px-2 py-1 rounded text-[11px] border ${on ? 'border-gold' : 'border-shell-600'}`}
