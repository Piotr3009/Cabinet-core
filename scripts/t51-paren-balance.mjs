#!/usr/bin/env node
// ─── PAREN BALANCE 0/0, BY SCRIPT (turn 51, CLAUDE.md iron rule 3) ──────────
//
//   *"LISP IS LAW — FIRST, for F5. The cup bore is a drilling; its depth rule
//   is born in `reference/lisp/` before any JS. No other LISP file moves.
//   Paren 13/13 at 0/0."*
//
// T50's script, RE-HEADED and re-pointed, because the claim is TONIGHT's and a
...
//      code or surface: F1's plan obstacles, F2/F3's settle, F4's measurement,
//      F6's light panel, F7's warehouse and F8's default. None of them has a
//      DRAWING law a kit could state — the kits draw a carcass, and what those
//      six move is a room, a layout, a number and two new screens.
//
//   3. THE CALL GRAPH. The cup-depth law is DEFINED once, in the file that owns
//      it, and mentioned in no other kit. A kit that had grown its own 11 would
//      balance perfectly and still be two answers to one question — which is
//      exactly the fault F5 is repairing on the application side.
//
// Usage:
//     node scripts/t51-paren-balance.mjs                    # every kit
//     node scripts/t51-paren-balance.mjs --against 31d2152  # …and the diff rule
//
...
export const T50_ROUTINE = 'SKY:cupThickness';
 ...
 * of material.
 */
export const T50_GATED = ['SKY:cupDepth', 'SKY:cupTooThin'];
...
export function gateFaults(dir = LISP_DIR) {
  ...
  for (const routine of T50_GATED) {
    ...
    if (!body.includes('SKY:cupThickness') && !body.includes('SKY:cupDepth')) {
  ...
  if (!/thickness AT THE CUP/i.test(text)) {
