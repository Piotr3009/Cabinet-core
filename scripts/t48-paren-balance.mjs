#!/usr/bin/env node
// ─── PAREN BALANCE 0/0, BY SCRIPT (turn 48, CLAUDE.md iron rule 3) ──────────
//
//   *"LISP IS LAW — FIRST, where a LISP owns the ground. F4's law is born in
//   `reference/lisp/KIT_LED_GROOVE.lsp` before any JS. No other LISP file is
//   touched — the infill has never had a drawing law of its own and two plain
//   rectangles need none; say so in the PR. Paren balance 13/13 at 0/0 by
//   script."*
//
// T47's script, RE-HEADED and re-pointed, because the claim is TONIGHT's and a
...
//      by character — this kit's sections are mostly prose and the prose is
//      full of brackets.
//
//   2. ONLY `KIT_LED_GROOVE.lsp` MOVED. `--against <ref>` asks git rather than
//      taking anybody's word. Tonight's other engine feature is the TOP INFILL
//      (F2), and it does NOT appear here on purpose: the infill has never had a
//      drawing law of its own, and what it becomes — two plain rectangles — is
//      `drawRect` twice. Writing a kit to say that would be inventing a law to
//      have one, which is the opposite of "LISP is law".
//
//   3. THE CALL GRAPH. `ledGrooveEndExtra` is DEFINED once, in the kit that
//      owns the groove, and CALLED by `drawLedGroove` in the same file — and
//      mentioned in no other kit. A constant somebody had copied into a second
//      file would balance perfectly and still be two answers to one question.
//
// Usage:
//     node scripts/t48-paren-balance.mjs                    # every kit
//     node scripts/t48-paren-balance.mjs --against f586f8c  # …and the diff rule
//
// `--against` takes THIS TURN'S OWN BASE — the commit this branch left from.
// `main` is older than T38 and would report eight turns of LISP work as
// tonight's.
//
// Zero dependencies.
...
/** The constant this turn TEACHES, and the one kit that owns it. */
export const T48_ROUTINE = 'ledGrooveEndExtra';
export const T48_KIT = 'KIT_LED_GROOVE.lsp';
/** Who may call it. The kit that defines it is also its only caller. */
export const T48_CALLERS = ['KIT_LED_GROOVE.lsp'];

/** Every file under reference/lisp/ this turn is allowed to have moved. */
export const T48_LISP_FILES = [T48_KIT];
...
export function routineCensus(dir = LISP_DIR) {
  const rows = readdirSync(dir)
    ...
    .map((name) => {
      ...
      const defines = (text.match(new RegExp(`\\(defun\\s+${T48_ROUTINE}\\b`, 'g')) || []).length;
      const mentions = (text.match(new RegExp(`${T48_ROUTINE}\\b`, 'g')) || []).length;
...
export function routineFaults(census = routineCensus()) {
  const faults = [];
  if (census.defined.length !== 1 || census.defined[0] !== T48_KIT) {
    faults.push(`${T48_ROUTINE} must be defined exactly once, in ${T48_KIT}`
      + ` — found in [${census.defined.join(', ') || 'nowhere'}]`);
  }
  for (const kit of T48_CALLERS) {
    if (!census.callers.includes(kit)) faults.push(`${kit} never calls ${T48_ROUTINE}`);
  }
  const strays = census.callers.filter((n) => n !== T48_KIT && !T48_CALLERS.includes(n));
  for (const s of strays) faults.push(`${s} mentions ${T48_ROUTINE} and is not one of the two kits`);
  return faults;
}
...
if (IS_CLI) {
  ...
  process.stdout.write(`\n${T48_ROUTINE} — defined in [${census.defined.join(', ')}], called from `
    + `[${census.callers.join(', ')}]\n`);
  const faults = routineFaults(census);
  for (const f of faults) process.stdout.write(`  ← ${f}\n`);
  bad += faults.length;
  if (!faults.length) process.stdout.write('  One constant, one kit, no second 10.0. ✓\n');
  ...
  if (againstAt >= 0 && argv[againstAt + 1]) {
    ...
    const offenders = changed.filter((c) => !T48_LISP_FILES.some((f) => c.file.endsWith(f)));
    if (offenders.length) {
      process.stdout.write(`\nIRON RULE 3: ${offenders.length} other kit(s) touched — only `
        + `${T48_LISP_FILES.join(', ')} may move.\n`);
