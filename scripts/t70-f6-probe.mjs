...
const chain = [
  ...
  ['2 · the event', 'src/retail/design/Options.jsx', /onPointerEnter=\{\(\) => A\.hoverBay\(z\.index\)\}/],
...
const verdict = cases.find((c) => c.after.drawn == null);
...
say();
if (!verdict) {
  say('  THE HOVER WORKS in both states. The element is');
  say(`  \`${chain[0].at}\` — the chip \`data-testid="inside-bay-N"\` in the INSIDE step —`);
  say(`  and the event is \`onPointerEnter\` (\`${chain[1].at}\`).`);
} else {
