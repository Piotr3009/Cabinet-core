#!/usr/bin/env node
// ─── THE BYTE-IDENTITY CONTRACT (turn 50, CLAUDE.md iron rule 2) ────────────
//
// T49's classifier, RE-HEADED rather than reused, because the header is the
// argument and the argument is this turn's. CLAUDE.md, 25.08:
//
//   *"BYTE-IDENTITY. `t50-classify` (copy `t49-classify.mjs`, runnable from
...
//
// T49 could claim the engine was never opened. T50 CANNOT: this turn cuts. So
// the claim is the harder one, and it is made feature by feature.
//
// ─── THE TWO CLAUDE.md NAMES, AND WHY EACH IS OUT OF REACH ──────────────────
//
//   F2  THE SHARE-OUT. `engine/shareOut.js` computes a PLAN off a RUN. The six
//       are computed by `computeCabinet(defaultParamsFor(id))` with no store,
//       no room and no second cabinet — `buildRuns([])` of nothing is nothing,
//       so there is no run to share out and the module is never even called.
//       It writes no parameter either (`--plan` below proves both).
//
//   F3  THE ROOM CLAMP. `engine/roomFit.js` REFUSES, at two surfaces, and it
//       answers `null` for every question asked with no room — which is every
//       question a golden asks. `--fit` runs that proof: each of the six is put
//       to the guard with no room and with a room it fits, and refused in
//       neither.
//
// ─── AND THE FIVE THAT DO CUT ───────────────────────────────────────────────
//
//   F5 the end panel stops at the slope, F6 the shaker's recess follows the
//   cut, F7 a hinge with no door goes, F11 the infill corner. Every one of them
//   is gated on something the six do not have: `cfg.slopeCut` is null in all
//   six (`--cut` opens that gate deliberately and prints what moves), and
//   `run_top_infill.sideMitre` is absent in all six (`--infill`, T48's census,
//   re-run rather than remembered).
//
// So six IDENTICAL is not a hope that this turn held its nerve; it is the shape
// of every gate above. If a golden moves, the answer is rule 2's: STOP and say
// so in the PR. Do not name a bucket for it.
//
...
//
//     node scripts/t50-classify.mjs --dump > /tmp/base.json     # on the base
//     node scripts/t50-classify.mjs --dump > /tmp/head.json     # on the branch
//     node scripts/t50-classify.mjs /tmp/base.json /tmp/head.json
//
// `--census` prints T42's rail census, `--infill` T48's top-infill census and
// `--cut` T47's open-gate proof, all three re-run rather than remembered.
// `--survivors` is T49's control audit, kept because F10 removes two menu
// ENTRIES tonight and iron rule 4 says the ACTIONS stay reachable.
//
// `--plan` and `--fit` are TONIGHT's own, and they are the two sentences above
...
// Tonight's two proofs (F2, F3) — the modules the goldens must be out of reach of.
...
/**
 * THE SHARE-OUT'S REACH (F2) — asked of the engine rather than of a comment.
 *
 * The six are single cabinets. `buildRuns` of one cabinet is one run of one
 * cabinet, and `shareOutPlan` of it never runs at all in a golden dump, because
 * `computeCabinet` does not import `engine/shareOut.js` — and even if it did,
 * the module writes nothing. Both halves are printed: what the module says
 * about each config, and whether the config's own params carry a single mark
 * of it.
 *
 * Six "no run, no mark" is the claim; anything else voids the header above.
 */
export function sharePlanCensus() {
  return STANDARD_CONFIGS.map((cfg) => {
    ...
    return {
      id: cfg.id,
      // With no wall to measure against there is nothing to share out at all.
      shares: Boolean(plan && plan.ok),
      fixed: widthFixed(unit),
      // The one thing a share-out could ever leave behind on a golden.
...
/**
 * THE ROOM CLAMP'S REACH (F3) — the same question, from the other side.
 *
 * `roomFitRefusal` answers `null` with no room, which is every question a
 * golden asks, and `null` again for a size that fits a room it is given. Both
 * are run: a rule that refused nothing because it was broken would pass the
 * first and fail the second.
 */
export function roomFitCensus() {
  const room = migrateRoom({ height: 3000, corners: rectCorners(6000, 4000) });
  return STANDARD_CONFIGS.map((cfg) => {
    const params = { ...defaultParamsFor(cfg.id, P), unit_num: '01' };
    const unit = {
      id: cfg.id, type: cfg.id, params, position: { wall: 0, x_mm: 0, rotation_deg: 0 },
    };
    const patch = { width: params.width, height: params.height, depth: params.depth };
    return {
      id: cfg.id,
      bare: roomFitRefusal({ unit, patch, room: null, profile: P }),
      inRoom: roomFitRefusal({
        unit, patch, room, profile: P,
      }),
      // …and it DOES refuse when it should, or it is not a guard.
      tooTall: roomFitRefusal({
        unit, patch: { height: 9000 }, room, profile: P,
      }),
    };
  });
}
...
if (IS_CLI) {
  ...
  if (argv.includes('--dump')) {
    ...
    process.stdout.write('THE SHARE-OUT\u2019S REACH \u2014 why F2 may rewrite a run without moving a golden\n\n');
    process.stdout.write(`${'config'.padEnd(12)}${'has a run to share'.padEnd(20)}${'width imposed'.padEnd(16)}width_fixed mark\n`);
    let clean = true;
    for (const row of sharePlanCensus()) {
      if (row.shares || row.mark != null) clean = false;
      process.stdout.write(
        `${row.id.padEnd(12)}${String(row.shares).padEnd(20)}`
        + `${String(row.fixed).padEnd(16)}`
        + `${row.mark == null ? '(none)' : String(row.mark)}\n`,
      );
    }
    process.stdout.write(`\n${clean
      ? 'CLEAN \u2014 not one of the six has a run to share out, and not one carries a mark of it. F2 cannot reach them.'
      : 'NOT CLEAN \u2014 a config has a run or a mark; the share-out is reaching a cabinet that is not in a run. STOP (iron rule 2).'}\n`);
    process.exit(clean ? 0 : 1);
  } else if (argv.includes('--fit')) {
    process.stdout.write('THE ROOM CLAMP\u2019S REACH \u2014 why F3 may refuse without moving a golden\n\n');
    process.stdout.write(`${'config'.padEnd(12)}${'bare (no room)'.padEnd(17)}${'in a 6\u00d74\u00d73 room'.padEnd(19)}9000 mm tall\n`);
    let clean = true;
    for (const row of roomFitCensus()) {
      // Refused with no room, or refused a size that fits: either is a guard
      // reaching a cabinet it has no business near. NOT refusing 9000 mm in a
      // 3000 mm room is the opposite fault and just as fatal.
      if (row.bare || row.inRoom || !row.tooTall) clean = false;
      process.stdout.write(
        `${row.id.padEnd(12)}${(row.bare ? 'REFUSED \u2190' : 'allowed').padEnd(17)}`
        + `${(row.inRoom ? 'REFUSED \u2190' : 'allowed').padEnd(19)}`
        + `${row.tooTall ? 'refused \u2713' : 'ALLOWED \u2190'}\n`,
      );
    }
    process.stdout.write(`\n${clean
      ? 'CLEAN \u2014 the guard refuses none of the six as they ship, and refuses 9000 mm in a 3000 mm room. It is a gate, and it is shut on the goldens.'
      : 'NOT CLEAN \u2014 the clamp is reaching a cabinet it has no business near, or it is not a gate at all. STOP (iron rule 2).'}\n`);
    process.exit(clean ? 0 : 1);
    ...
    process.stdout.write('EXPECTED BUCKETS: none. This turn DOES cut — F5\u2013F7 and F11 all move board — but every one of them is gated on something the six do not have: `cfg.slopeCut` is null in all six (--cut opens that gate deliberately) and `run_top_infill.sideMitre` is absent in all six (--infill). F2 has no run to share (--plan) and F3 refuses nothing with no room (--fit).\n');
    process.stdout.write('IF A GOLDEN MOVED:  iron rule 2 \u2014 the share-out or the clamp is reaching a cabinet that is not in a run. That is a BUG, not a licence. Stop and say so.\n');
