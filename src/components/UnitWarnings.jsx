...
export default function UnitWarnings({ unitId, only = null, compact = false }) {
  ...
  return (
    <ul className={compact ? 'space-y-0.5' : 'space-y-1 mt-2'} data-unit-warnings={rows.length}>
      {rows.map((w, i) => (
        <li
          // eslint-disable-next-line react/no-array-index-key
          key={`${w.code}-${i}`}
          data-warning-code={w.code}
          className="flex items-start gap-1.5 text-[11px] text-status-warn leading-snug"
        >
