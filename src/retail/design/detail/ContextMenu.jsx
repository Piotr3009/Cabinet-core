...
export default function ContextMenu() {
  ...
  return (
    <div
      ref={box}
      className={`fixed ${LAYER_CLASS.menu} pbi-re-w180 pbi-re-panel pbi-re-py1`}
    ...
    >
      ...
      <div
        className="pbi-re-px3 pbi-re-py1 pbi-re-t11 pbi-re-quiet pbi-re-line-b pbi-re-hair pbi-re-cursor-move pbi-re-noselect"
      ...
      {groups.map((group, gi) => (
        <div key={group.id}>
          {gi > 0 && <div className="pbi-re-mx3 pbi-re-my1 pbi-re-line-t pbi-re-hair-gold-soft" data-menu-divider={group.id} />}
          {group.actions.map((a) => (
            <button
              key={a.id}
              type="button"
              ...
              disabled={a.disabled}
              className={`pbi-re-wfull pbi-re-left pbi-re-tsm pbi-re-fade pbi-re-dis-dim
                pbi-re-dis-nope pbi-re-fill-hover-en ${
                a.danger ? 'pbi-re-bad' : 'pbi-re-ink-1'} ${
                // F6.1: "Edit cabinet" FIRST and FRAMED — a bordered, standout
                // entry, because it is the one a joiner reaches for once the
                // cabinet is standing.
                a.framed
                  ? 'pbi-re-my1 pbi-re-mx2 pbi-re-px2 pbi-re-py15 pbi-re-round pbi-re-line pbi-re-hair-gold pbi-re-gold pbi-re-wcalc16'
                  : 'pbi-re-px3 pbi-re-py15'}`}
              title={a.hint || ''}
              onClick={() => {
                a.run();
            ...
            >
              ...
              {a.checked === undefined
                ? a.label
                : (
                  <span className="pbi-re-row pbi-re-mid pbi-re-gap-15">
                    <span className={a.checked ? 'pbi-re-gold' : 'pbi-re-quiet'}>{a.checked ? '✓' : '·'}</span>
                    <span>{a.label}</span>
                  </span>
                )}
            </button>
