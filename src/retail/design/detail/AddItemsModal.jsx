...
export default function AddItemsModal() {
  ...
  return (
    <Modal
      ...
      width="pbi-re-w320"
      footer={<button type="button" className="pbi-re-btn-gold" onClick={() => { setZoneHint(null); closeModal(); }}>Done</button>}
    >
      <div className="pbi-re-stack-3">
        <section className="pbi-re-stack-1">
          <span className="pbi-re-txs pbi-re-caps pbi-re-track pbi-re-ink-2">This cabinet</span>
          <div className="pbi-re-grid pbi-re-grid-3 pbi-re-gap-2">
            {dims.map((d) => (
              <label key={d.key} className="pbi-re-block">
                <span className="pbi-re-fieldlabel">{d.label}</span>
                <NumberField
        ...
        <section className="pbi-re-stack-1">
          <span className="pbi-re-txs pbi-re-caps pbi-re-track pbi-re-ink-2">What goes inside</span>
          ...
          {type.supports?.doors !== false && (
            <button
              type="button"
              className="pbi-re-btn pbi-re-wfull pbi-re-mt1"
              data-add-doors="1"
              disabled={hasDoors}
