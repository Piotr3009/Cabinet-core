      ...
      case 'shelf-type':
        return (
          <Field key={key} label="Type">
            <div className="pbi-re-row pbi-re-mid pbi-re-gap-1" data-shelf-type="1">
              {SHELF_TYPES.filter((t) => t.id === 'fix' || t.id === 'adjustable').map((t) => (
                <button
                  key={t.id}
                  type="button"
                  className={`pbi-re-btn pbi-re-grow pbi-re-px2 pbi-re-t11 ${shelfTypeOf(item) === t.id ? 'pbi-re-hair-gold pbi-re-ink' : ''}`}
        ...
        return (
          <Field key={key} label="Set back">
            <div className="pbi-re-stack-1">
              <div className="pbi-re-row pbi-re-mid pbi-re-gap-1" data-setback-chips="1">
                {backChips.map((c) => (
                  <button
                    key={c.id}
                    type="button"
                    className={`pbi-re-btn pbi-re-grow pbi-re-px2 pbi-re-t11 ${backNow === c.mm ? 'pbi-re-hair-gold pbi-re-ink' : ''}`}
              ...
              <NumberField
                className="pbi-re-input pbi-re-right pbi-re-wfull"
                min={bounds.min}
                max={bounds.max}
                value={backNow}
